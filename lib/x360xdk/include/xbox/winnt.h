...
#define CONTEXT_VECTOR          0x00000010L

#define CONTEXT_FULL (CONTEXT_CONTROL | CONTEXT_FLOATING_POINT | CONTEXT_INTEGER | CONTEXT_VECTOR)
...
typedef struct _CONTEXT {
    ...
    DWORD UserModeControl;              // User mode control state
    DWORD Fill;
...
typedef struct _STACK_FRAME_HEADER {    // GPR 1 points here
    DWORD BackChain;                    // address of previous frame
    DWORD CompilerReserved;             // used by C++ EH support
    DWORD CalleeOwned1;
    DWORD CalleeOwned2;
    ULONGLONG Parameter0;               // storage for callee parameters
    ULONGLONG Parameter1;               //     (must be 16 byte aligned)
    ULONGLONG Parameter2;
    ULONGLONG Parameter3;
    ULONGLONG Parameter4;
    ULONGLONG Parameter5;
    ULONGLONG Parameter6;
    ULONGLONG Parameter7;
...
//
// Define macros to read little endian values from the image header.
//

#if defined(_M_PPCBE)
#define IMAGE_READ_DWORD(Pointer)           (_byteswap_ulong(*(Pointer)))
#define IMAGE_READ_WORD(Pointer)          (_byteswap_ushort(*(Pointer)))
#else
#define IMAGE_READ_DWORD(Pointer)           (*(Pointer))
#define IMAGE_READ_WORD(Pointer)          (*(Pointer))
#endif
...
#define IMAGE_FIRST_SECTION( ntheader ) ((PIMAGE_SECTION_HEADER)        \
    ((ULONG_PTR)ntheader +                                              \
     ...
     IMAGE_READ_WORD(&((PIMAGE_NT_HEADERS)(ntheader))->FileHeader.SizeOfOptionalHeader) \
    ))
