...
//
// Win32 SMB server returns file data to SMB client in this format.
//
...
InterlockedIncrement64(
    IN OUT LONG64 volatile *lpAddend
    );
...
#define	AcquireLockBarrier() __emit(0x7C2004AC) //__lwsync()
#define	ReleaseLockBarrier() __emit(0x7C2004AC) //__lwsync()

#else
// The Interlocked operations are already full memory barriers on Windows so
// no additional barriers are needed for the Acquire and Release forms.
#define	AcquireLockBarrier() 
#define	ReleaseLockBarrier() 
#endif

//
// Acquire and Release versions of Interlocked functions for easier safe
// usage. These are the Acquire and Release variations defined for Windows.
// These versions add a memory barrier as appropriate. Unlike Windows the
// non Acquire/Release variations do not include a memory barrier.
//
...
{
    LONG result = InterlockedIncrement(lpAddend);
    // AcquireLockBarrier must be after we do the interlocked operation
    AcquireLockBarrier();
    return result;
}
...
{
	LONG result;
    // ReleaseLockBarrier must be before we do the interlocked operation
    ReleaseLockBarrier();
    result = InterlockedIncrement(lpAddend);
    return result;
}
...
{
    LONG result = InterlockedDecrement(lpAddend);
    // AcquireLockBarrier must be after we do the interlocked operation
    AcquireLockBarrier();
    return result;
}
    ...
    )
{
    ...
    // ReleaseLockBarrier must be before we do the interlocked operation
    ReleaseLockBarrier();
    result = InterlockedDecrement(lpAddend);
    return result;
}
...
{
    LONG64 result = InterlockedCompareExchange64(Destination, Exchange, Comperand);
    // AcquireLockBarrier must be after we do the interlocked operation
    AcquireLockBarrier();
    return result;
}
...
{
    PVOID result = InterlockedCompareExchangePointer(Destination, Exchange, Comperand);
    // AcquireLockBarrier must be after we do the interlocked operation
    AcquireLockBarrier();
    return result;
}
...
#if defined(_SLIST_HEADER_)

#define InitializeSListHead(ListHead)   ((ListHead)->Alignment = 0)
#define QueryDepthSList(ListHead)       ((USHORT)(ListHead)->Depth)
...
// Acquire and Release versions of InterlockedSList functions for easier safe
// usage. These two functions are not available for Windows.
// These versions add a memory barrier as appropriate. Unlike Windows the
// non Acquire variation of InterlockedPopEntrySList does not include a memory barrier.

__inline 
PSLIST_ENTRY
__cdecl
InterlockedPopEntrySListAcquire(
	PSLIST_HEADER ListHead
    )
{
    PSLIST_ENTRY result = InterlockedPopEntrySList(ListHead);
    // AcquireLockBarrier must be after we do the interlocked operation
    AcquireLockBarrier();
    return result;
}
...
PSLIST_ENTRY
__cdecl
InterlockedPushEntrySListRelease(
    PSLIST_HEADER ListHead,
    PSLIST_ENTRY ListEntry
    )
{
	PSLIST_ENTRY result;
	// InterlockedPushEntrySList includes a release memory barrier
	// so the Release variant does nothing extra, and exists only to
	// make this plain to the reader.
...
#define GetFiberData() (*(LPVOID *)(GetCurrentFiber()))
...
#define HasOverlappedIoCompleted(lpOverlapped) \
    (*((volatile ULONG_PTR*)(&(lpOverlapped)->Internal)) != STATUS_PENDING)
