	...
	*bIsMoving = false;
	...
	if ( bMoving )
	{
		...
		else
		{
			...
			flReturn = seclamp( flReturn, 0.01f, 10.0f );
		}
...
void CMultiPlayerAnimState::EstimateYaw( void )
{
	...
	else
	{
		...
		m_PoseParameterData.m_flEstimateYaw = seclamp( m_PoseParameterData.m_flEstimateYaw, -180.0f, 180.0f );
	}
}
...
#define FADE_TURN_DEGREES 60.0f
	...
	flScale = seclamp( flScale, 0.01f, 1.0f );
