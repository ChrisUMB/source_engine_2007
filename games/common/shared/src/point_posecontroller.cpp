...
{
	m_fPoseValue = seclamp( fValue, 0.0f, 1.0f );
...
{
	m_fInterpolationTime = seclamp( fValue, 0.0f, MAX_POSE_INTERPOLATION_TIME );
}
...
{
	m_fCycleFrequency = seclamp( fValue, -MAX_POSE_CYCLE_FREQUENCY, MAX_POSE_CYCLE_FREQUENCY );
}
...
{
	m_fFModTimeOffset = seclamp( fValue, -1.0f, 1.0f );
}

void CPoseController::SetFModRate( float fValue )
{
	m_fFModRate = seclamp( fValue, -MAX_POSE_FMOD_RATE, MAX_POSE_FMOD_RATE );
}

void CPoseController::SetFModAmplitude( float fValue )
{
	m_fFModAmplitude = seclamp( fValue, 0.0f, MAX_POSE_FMOD_AMPLITUDE );
}

void CPoseController::RandomizeFMod( float fExtremeness )
{
	fExtremeness = seclamp( fExtremeness, 0.0f, 1.0f );
	...
	case POSECONTROLLER_FMODTYPE_NOISE:
		{
			...
			m_fCurrentFMod = seclamp( m_fCurrentFMod, -m_fFModAmplitude, m_fFModAmplitude );
