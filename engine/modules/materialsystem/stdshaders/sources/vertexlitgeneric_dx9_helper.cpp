...

{
	...
	if ( pShader->IsSnapshotting() || (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
	{
		...
		if ( pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) ) )
		{
			...
			if ( bDistanceAlpha )
			{
				...
				if ( bScaleEdges || bScaleOutline )
				{
					...
					if ( bScaleEdges )
					{
						...
						flSoftStart = seclamp( flMid + flResScale * ( flSoftStart - flMid ), 0.05, 0.99 );
						flSoftEnd = seclamp( flMid + flResScale * ( flSoftEnd - flMid ), 0.05, 0.99 );
					}
					...
					if ( bScaleOutline )
					{
						...
						flOutlineStart1 = seclamp( flMidS + flResScale * ( flOutlineStart1 - flMidS ), 0.05, 0.99 );
						float flMidE = 0.5 * ( flOutlineEnd1 + flOutlineEnd0 );
						flOutlineEnd1 = seclamp( flMidE + flResScale * ( flOutlineEnd1 - flMidE ), 0.05, 0.99 );
					}
			...
			if ( bHasEnvmap )
			{
				pContextData->m_SemiStaticCmdsOut.SetEnvMapTintPixelShaderDynamicStateGammaToLinear( 0, info.m_nEnvmapTint );
			}
			bool bHasEnvmapMask = (!bHasFlashlight || IsX360() ) && info.m_nEnvmapMask != -1 && params[info.m_nEnvmapMask]->IsTexture();
			if ( bHasEnvmapMask )
			{
								...
								CBasePerMaterialContextData **pContextDataPtr )
{
	...
	{
		DrawSkin_DX9( pShader, params, pShaderAPI, pShaderShadow, info, vertexCompression );
		return;		
	}
